...
# https://github.com/docker-library/openjdk/blob/master/13/jdk/slim/Dockerfile
...
# custom configuration for register
...
ADD dockerconf/supervisor.conf /etc/supervisor.conf
# supervisor nginx conf
ADD dockerconf/nginx.sv.conf /etc/supervisor/conf.d

# supervisor tomcat conf
ADD dockerconf/tomcat.sv.conf /etc/supervisor/conf.d


# ldregistry setup
RUN mkdir -p /var/opt/ldregistry/

# nginx content and config
RUN ln -s /usr/share/nginx/html /usr/share/nginx/www

ADD dockerconf/reg.nginx.conf /etc/nginx/conf.d
RUN rm /etc/nginx/conf.d/default.conf
RUN mkdir -p /var/opt/nginx/cache
RUN chown nginx:nginx /var/opt/nginx/cache

ADD dockerconf/sudoers.conf /etc/sudoers.d

# configure tomcat for LD-Registry
RUN mv /usr/local/tomcat/webapps/ROOT /usr/local/tomcat/webapps/Default
RUN  apt-get install -y curl
RUN curl https://s3-eu-west-1.amazonaws.com/ukgovld/release/com/github/ukgovld/registry-core/2.1.0/registry-core-2.1.0.war -o /usr/local/tomcat/webapps/ROOT.war

ADD ldregistry /opt/ldregistry
